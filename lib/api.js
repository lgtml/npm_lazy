...
function noCacheRequest(req, res) {

  function sendUTF8 (data) {
  ...
  var options = {
    method: req.method,
    headers: req.headers,
    send: sendUTF8.bind(this)
  };

  console.log('URL', req.url);
  Package.genericRequest(req.url, options, function(err, data) {
    console.log('completed no cache request', req.url);
    res.end();
  });
...
api.get(new RegExp('^/-/(.+)$'), noCacheRequest);
api.get(new RegExp('^/(.+)/-(.*)$'), noCacheRequest);
